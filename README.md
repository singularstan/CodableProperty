...
1. Codable struct contains either autogenerated or custom defined enum **CodingKeys**. Default implementation demand every name defined in CodingKeys must exist in JSON. If you want to implement “stable” strategy you have to use method **decodeIfPresent(_: forKey:)**. If you have to dial with response where JSON contains only part of mandatory keys or for some reason response depends on some mystery events from another galaxy and backend developer is inaccessible or thinks it should be so you have a problem. **Codable property** implements the next strategy: **it collects the information about declared properties and then tries to find each of them in JSON response**. It rises an exception only in case when property was marked as mandatory. As a result **init(from decoder: Decoder)** constructs result struct even using empty JSON and you can check if the property has default value or not.
